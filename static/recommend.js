$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = '8306a2c064c0db7202d1d0970f58dd0c';
    var title = $('.movie').val();
    if (title=="") {
...
function movie_recs(movie_title,movie_id,my_api_key){
  $.ajax({
    ...
    success: function(recs){
      if(recs=="Sorry! The movie you requested is not in our database. Please check the spelling or try with another movies title"){
        $('.fail').css('display','block');
        $('.results').css('display','none');
        $("#loader").delay(500).fadeOut();
      }
